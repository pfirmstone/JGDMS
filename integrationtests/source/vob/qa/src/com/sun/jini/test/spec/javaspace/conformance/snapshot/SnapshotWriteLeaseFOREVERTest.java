 ...
 * <code>Lease.FOREVER</code> lease time without transactions:
...
public class SnapshotWriteLeaseFOREVERTest extends SnapshotAbstractTestBase {
    ...
    public void run() throws Exception {
        ...
        // crate snapshots of sample entries
        snapshot1 = space.snapshot(sampleEntry1);
        snapshot2 = space.snapshot(sampleEntry2);

        /*
         * write 1-st entry without transactions using it's snapshot
         * with Lease.FOREVER lease time
         */
        space.write(snapshot1, null, Lease.FOREVER);
        ...
        /*
         * write 2-nd entry without transactions using it's snapshot
         * with Lease.FOREVER lease time
         */
        space.write(snapshot2, null, Lease.FOREVER);
        ...
        logDebugText("Write of " + sampleEntry2
                + " using it's snapshot with Lease.FOREVER"
                + " lease time works as expected.");

        /*
         * write 1-st entry using it's snapshot
         * with Lease.FOREVER lease time again
         */
        space.write(snapshot1, null, Lease.FOREVER);
