 ...
 */
public class PingAckMessage implements Message {
     ...
     */
    public PingAckMessage setCookie(short cookie) {
     ...
     * to the format for a PingAck message
     *
    ...
    private void check(byte[] message) throws ProtocolException {
        ...
        if ((received[0]|reference[0])!=reference[0]) {
            throw new ProtocolException("The message received: "
                + Util.convert(received) + " is not of PingAck message type");
        }
        //check that the reserved byte is not used
        if ((received[1]|0x00)!=0x00) {
            throw new ProtocolException("The received PingAck message makes"
                + " use of reservered bits");
        }
        //check that the received cookie matches the expected cookie
        if (received[2]!=reference[2]||received[3]!=reference[3]) {
            throw new ProtocolException("The cookie received from PingAck: "
                + Util.convert(new byte[]{received[2],received[3]}) +
                " does not match the cookie expected: " + Util.convert(
                new byte[] {reference[2],reference[3]}));
        }
    }
}
