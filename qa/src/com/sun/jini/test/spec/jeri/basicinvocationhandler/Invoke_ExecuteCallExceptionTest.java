 ...
 *   method when an exception is thrown during a call
 *   to ObjectEndpoint.executeCall.
 *
 * Test Cases
 ...
 *      Throwable executeCallException
 ...
 *          -executeCall method throws executeCallException
 ...
 *     6) invoke FakeObjectEndpoint.setExecuteCallException
 *        passing in executeCallException
 *     7) invoke FakeOutboundRequest.setDeliveryStatusReturn
 *        passing in deliveryStatus
 *     8) invoke the method on the dynamic proxy
 *     9) assert executeCallException is thrown directly or
 *        appropriately wrapped
 * </pre>
 */
public class Invoke_ExecuteCallExceptionTest extends AbstractInvokeTest {
    ...
    public void run() throws Exception {
        ...
        for (int h = 0; h < deliveryStatusCases.length; h++) {
            ...
            for (int i = 0; i < cases.length; i++) {
                ...
                Throwable executeCallException = cases[i];
                logger.log(Level.FINE,"test case " + (counter++) + ": " 
                    + "deliveryStatus:" + deliveryStatus
                    + ",exception:" + executeCallException);
                logger.log(Level.FINE,"");

                iterator.init();
                request.setDeliveryStatusReturn(deliveryStatus);
                objectEndpoint.setExecuteCallException(
                    executeCallException);
