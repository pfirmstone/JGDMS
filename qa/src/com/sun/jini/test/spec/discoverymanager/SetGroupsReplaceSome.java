 ...
 * the original set of groups to discover, the lookup discovery manager will
 ...
 *         replaced in the lookup discovery manager
 * </ul><p>
 * 
...
public class SetGroupsReplaceSome extends Discovered {
    ...
    protected boolean   alternateReplacement = true;
     ...
     *    <li> invokes the setGroups method on the lookup discovery 
    ...
    public void run() throws Exception {
        ...
        if((newGroupsToDiscover != null)&&(newGroupsToDiscover.length<=0)){
            ...
            GroupsUtil.displayGroupSet(newGroupsToDiscover,
                                       "   newGroup",Level.FINE);
        }//endif
        discoveryMgr.setGroups(newGroupsToDiscover);
        ...
        for(int i=0;i<list.size();i++) {
            ...
            if( ((i%2) == 0) || !alternate ) {//index is even or removeAll
                for(int j=0;j<curGroups.length;j++) {
                    newDiscoverList.add(new String(curGroups[j]+"_new"));
                }//end loop(j)
