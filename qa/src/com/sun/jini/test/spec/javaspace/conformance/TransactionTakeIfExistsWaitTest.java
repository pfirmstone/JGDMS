...
public class TransactionTakeIfExistsWaitTest extends AbstractTestBase {
    ...
    public void run() throws Exception {
        ...
        /*
         * takeIfExists an entry from the space with timeout less then
         * transaction's lease time, check that takeIfExists request will
        ...
        if (result != null) {
            throw new TestException(
                    "performed takeIfExists of " + sampleEntry1 + " with "
        ...
        /*
         * takeIfExists an entry from the space with timeout greater
         * then transaction's lease time, check that takeIfExists request
        ...
        if (result == null) {
            throw new TestException(
                    "performed takeIfExists of " + sampleEntry1 + " with "
                    + timeout2 + " timeout, expected non null but took"
                    + " null result.");
        }
        ...
        // takeIfExists any entry from the space
        result = (SimpleEntry) space.takeIfExists(null, null, timeout2);
        curTime2 = System.currentTimeMillis();
        ...
        if (result == null) {
            throw new TestException(
                    "performed takeIfExists with null template and "
                    + timeout2 + " timeout, expected non null but took"
                    + " null result.");
        }
