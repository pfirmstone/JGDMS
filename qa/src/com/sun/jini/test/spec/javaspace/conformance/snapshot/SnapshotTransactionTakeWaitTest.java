...
public class SnapshotTransactionTakeWaitTest extends SnapshotAbstractTestBase {
    ...
    public void run() throws Exception {
        ...
        /*
         * take an entry from the space using it's snapshot
         * with timeout less then transaction's lease time,
         * check that take request will return null value
         * in time specified in read request
         */
        curTime1 = System.currentTimeMillis();
        result = (SimpleEntry) space.take(snapshot1, null, timeout1);
        curTime2 = System.currentTimeMillis();

        if (result != null) {
            throw new TestException(
                    "performed take with template " + sampleEntry1
                    + " using it's snapshot and " + timeout1
         ...
         * check that take request will return non null entry
         * after transaction's expiration
         */
        result = (SimpleEntry) space.take(snapshot1, null, timeout2);
        curTime2 = System.currentTimeMillis();

        if (!sampleEntry1.equals(result)) {
            throw new TestException(
                    "performed take with template " + sampleEntry1
                    + " using it's snapshot and " + timeout2
                    + " timeout, expected " + sampleEntry1
        ...
        logDebugText("Take with " + (timeout2 * 2)
