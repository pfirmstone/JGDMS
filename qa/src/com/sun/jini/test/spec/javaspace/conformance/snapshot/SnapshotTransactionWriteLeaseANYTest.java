 ...
 * <code>Lease.ANY</code> value for lease time within the non null transaction:
...
public class SnapshotTransactionWriteLeaseANYTest
    ...
    public void run() throws Exception {
         ...
         */
	space.write(snapshot1, txn, Lease.ANY);
        ...
        if (result == null) {
            throw new TestException(
                    ...
                    + " time within the non null transaction, written entry"
	...
	space.write(snapshot2, txn, Lease.ANY);
        ...
        /*
         * write 1-st entry using it's snapshot with Lease.ANY value
         * for lease time again within the same non null transaction
         */
	space.write(snapshot1, txn, Lease.ANY);
        // check that original entry has not been changed
        if (!origEntry1.equals(sampleEntry1)) {
            throw new TestException(
                    "performed 2-nd write operation for snapshot of the"
                    + " same entry with Lease.ANY value for lease time"
        ...
        if (result == null) {
            throw new TestException(
                    ...
                    + sampleEntry1 + " using it's snapshot with Lease.ANY"
        ...
        logDebugText("2-nd write of the same " + sampleEntry1
                + " using it's snapshot with Lease.ANY lease time within"
                + " the same non null transaction works as expected.");
