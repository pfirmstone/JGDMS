 ...
 * the <code>setLocators</code> method contains at least one <code>null</code>
 ...
 *
 */
public class SetLocatorsNullElement extends AddLocatorsNullElement {
     ...
     *          of locators
     *     <li> invokes setLocators with a set of locators in which at least
     *          one element is null
	...
	if (!LocatorsUtil.compareLocatorSets(lldLocs, configLocs,Level.FINE)) {
        ...
        /* Replace locators with set containing at least 1 null element */
        try {
            lld.setLocators(nullLocs);
            String errStr = new String("no NullPointerException");
            logger.log(Level.FINE, errStr);
            throw new TestException(errStr);
        } catch(NullPointerException e) {
            logger.log(Level.FINE, 
		     "NullPointerException on locator replacement as expected");
        }
    }//end run

}//end class SetLocatorsNullElement
