    ...
    public void run() throws Exception {
        ...
        for (int i = 0; i < cases.length; ++i) {
            ...
            InvocationConstraints constraints = new InvocationConstraints(
                    ic, null);
            StringMethodDesc md = null;
            String[] returnedTypes = null;
            ...
            if (testCase == case3arg) {
                ...
                for (int j = 0; j < types.length; ++j) {
                    if (returnedTypes[j] != types[j].getCanonicalName()) {
                        throw new TestException(
            ...
            if (testCase == case3arg) {
                md = callConstructor(testCase, name, types, constraints);
                returnedTypes = md.getParameterTypes();
                String[] returnedTypes2 = md.getParameterTypes();
                if (Arrays.equals(returnedTypes, returnedTypes2)) {
                    throw new TestException(
