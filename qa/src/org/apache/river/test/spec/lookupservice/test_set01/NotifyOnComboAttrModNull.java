 ...
 */
public class NotifyOnComboAttrModNull extends QATestRegistrar {
     ...
     *  the loaded attributes; where each instance from the first set is
     *  non-initialized (null fields) and each instance from the second is
     *  initialized (non-null fields). Computes the maximimum time to wait
     *  for all events to arrive from lookup; based on that event wait time,
     *  computes and sets values for the maximum duration of all service and
     *  event leases. The maximum duration should be large enough to
     ...
     *  service classes; then registers each service instance with the lookup 
     *  service. Creates another set of instances of the service classes 
     *  that will be used to determine equality in the method objEquals() 
     *  from the class QATestUtils.SrvcAttrTuple. Adds each attribute 
     *  instance from the first set to each registered service. For each 
     ...
     *  requests a handback which contains the corresponding service/attribute 
		...
		proxy.notify(tmpl[i][j],
				 ...
				 (new QATestUtils.SrvcAttrTuple
				     (srvcItems[nSrvcsPerClass*i].service,
				      modAttrs[j][0])).convertToMarshalledObject(),
			     Long.MAX_VALUE);
	    }
	}
        return this;
