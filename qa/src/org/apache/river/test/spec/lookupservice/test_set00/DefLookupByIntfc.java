 ...
 */
public class DefLookupByIntfc extends QATestRegistrar {
     ...
     */
     private boolean setsAreEqual(int intfcIndx, Object [] srvcM) throws TestException
    {
        int     si;
        int     i,j,k;
        boolean inSet;
        for (i=0; i<srvcM.length; i++) 
	{
            inSet = false;
            for (j=0; (   (inSet==false)
                        &&(j<srvcM.length/nInstancesPerClass)); j++) 
	    {
                ...
                for (k=0; ((inSet==false)&&(k<nInstancesPerClass)); k++) 
		{
			...
			throw new TestException(
			    "RemoteException thrown while attempting to obtain service proxy", ex);
		    }
