 ...
 * successfully employ the multicast and unicast discovery protocols on behalf
 * of a client to discover a number of pre-determined lookup services when the
 * lookup discovery manager is configured to discover all of those lookup
 * services through locator discovery, and is additionally configured to
 * discover ALL_GROUPS; and then for each discovered lookup service, send to
 * the client's listener, the appropriate discovery event with the appropriate
 * contents.
 *
 ...
 *         and configured to discover all of the lookups by locator discovery
 ...
 * expected contents.
 */
public class LocsDiscoveredAll extends DiscoveredAll {
