...
public abstract class LeaseGrantTestBase extends TestBase implements Test {
	...
	if (underTest.getExpiration() == Long.MAX_VALUE &&
	 ...
	 // Parse command line
	 exact = getConfig().getBooleanConfigVal("org.apache.river.test.share.exact", false);
	 clip  = getConfig().getLongConfigVal("org.apache.river.test.share.clip", -1);
	 slop  = getConfig().getLongConfigVal("org.apache.river.test.share.slop", 1000);

	 final String durStr = getConfig().getStringConfigVal("org.apache.river.test.share.duration", null);
	 if (durStr == null) {
