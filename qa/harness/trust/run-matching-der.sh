...
LOGDIR="$ROOT/JGDMS/dist/target/matching-der-logs"
mkdir -p "$LOGDIR"
RUNLOG="$LOGDIR/run.log"

echo "== run-matching-der (DER export) =="
...
echo "-- running matching test under -Dharness.configs=der --"
cd "$QA"
ant -Dharness.configs=der -Drun.tests="$TEST_TD" run-tests 2>&1 | tee "$RUNLOG"
