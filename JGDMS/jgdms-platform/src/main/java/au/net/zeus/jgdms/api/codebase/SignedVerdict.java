 ...
 * The class is {@code final} so that subclasses cannot weaken the invariants.
 *
...
public final class SignedVerdict implements Serializable {
    ...
    public static void serialize(PutArg arg, SignedVerdict sv) throws IOException {
        arg.put(CODEBASE_URLS, sv.codebaseUrls.clone());
        arg.put(VERDICT,       sv.verdict);
        arg.put(TIMESTAMP,     sv.timestamp);
        arg.put(SIGNATURE,     sv.signature.clone());
        arg.writeArgs();
    }
    ...
    /**
     * The ordered set of codebase URLs that were analysed, stored as RFC3986
     * URI strings for safe serialization.  The order is significant: the
     * canonical byte representation that is signed includes the URLs in
     * iteration order.
     *
     * @serial
     */
     ...
     */
    public SignedVerdict(GetArg arg) throws IOException, ClassNotFoundException {
        this(arg, check(arg));
    }

    private SignedVerdict(GetArg arg, boolean checked) throws IOException, ClassNotFoundException {
     ...
     *        that were analysed; must be non-null and non-empty
     ...
     */
    public SignedVerdict(Uri[] codebaseUrls,
                         VerdictType verdict,
                         long timestamp,
                         byte[] signature) {
     ...
     * that were analysed.  Callers that need a {@link java.net.URL} for
