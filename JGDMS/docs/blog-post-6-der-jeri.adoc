...
JDK-internal mechanism. None of which is a dismissal of JPMS as a whole: its
strong encapsulation was a genuine security advance — it shrank the platform's
trusted, reflectively-reachable surface to `java.base` and finally walled off the
internal APIs (`sun.misc.Unsafe` and kin) that were a perennial source of exploits
and fragile upgrades. The criticism here is deliberately narrow: the same rigidity
that makes that encapsulation a win makes _version_ coexistence the wrong shape for
the open library and application layer — a versioning critique, not a verdict on
the module system.
