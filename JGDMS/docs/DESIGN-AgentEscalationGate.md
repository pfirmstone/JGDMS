...
*4. **The one-shot grant is made replay-proof by a nonce, and restore semantics are stated.** The old
one-shot + TTL rested on TTL + post-op revoke (temporal); nothing stopped a snapshotted/restored or
clock-rewound agent from re-presenting a prior approval. Each approval now carries a
nonce consumed on first install; a replayed `Outcome` installs nothing, and an
...
decision cache **and** the `SecurityPolicyWriter`/polpAudit recorder) and a new `impliesEphemeral`
(grant-only probe for ephemeral authority). Ephemeral grants are invisible to `implies` and reported
only by `impliesEphemeral`, which the SM consults **after** a failed `implies`, outside the protected
per-PD recording method. One relation split yields three properties at once: an ephemeral grant is never
...
as a `DelegatePermission` (the ephemeral capability grant, now hack-free). `clearCache()`/`refresh()` keeps
its existing role for the reusable-revocable class only (e.g. a smart-proxy `DelegatePermission`
discarded mid-life). New §3 subsection "Ephemeral authority: `implies` vs `impliesEphemeral`"; §1 T3, §5,
§6-9 updated; tests grow to C1–C16. The `impliesEphemeral` addition to `java.security.Policy` (default
...
> **Invariant T3 — one-shot, not a mode, and replay-proof.** An approved escalation authorises a
> single operation (one-shot cardinality), never a standing capability. Two distinct things can outlive the grant, and
...
>   never fires on the eventless *expiry* path), ephemeral decisions are **never cached in the first
>   place**: the policy reports an ephemeral decision only via `impliesEphemeral`, which the SM consults *after* a
...
>   through a method-guard delegate, so each use re-checks; because the ephemeral decision is reported by
>   `impliesEphemeral` (never cached), that per-use re-check re-reads lease liveness every time and
...
> nonce consumed on first install (§3), so a replayed or restored approval installs
> nothing.
...
**Vocabulary — three orthogonal axes.** The mechanisms below bound an escalation along three
*independent* dimensions; conflating them is what made the old name `impliesOnce` misleading (it
named a *cardinality* where the policy relation actually reports a *visibility* property — hence the
rename to `impliesEphemeral`). The rest of this document uses these terms precisely:

- **Ephemerality (visibility axis).** An **ephemeral** grant's decision is *never cached* by the
  authorization SM and *never recorded* into generated (`polpAudit`) least-privilege policy — exactly
  what the policy's `impliesEphemeral` relation reports (vs `implies` for stable authority). This is
  independent of how many times, or for how long, the grant may be exercised.
- **Cardinality (count axis).** How many exercises a grant permits: **one-shot** (exactly one — the
  gate's default), a **countdown-latch** (N exercises, atomically decremented; one-shot is the N = 1
  case), or **one-shot + TTL** (one exercise *or* lease expiry, whichever comes first — Option (ii)'s
  `LeasedDelegation`).
- **Replay-protection (approval axis).** The **nonce** (`ApprovalId`): a single-use, non-serializable
  token, consumed on first install, that makes the *approval* replay/restore-proof. A property of the
  approval act, not of the granted authority.

An escalation grant is typically all three at once — e.g. **one-shot** *and* **ephemeral** *and*
installed under a **nonce** — but each property is set, enforced, and reasoned about separately.
...
| approval → **nonce-guarded one-shot install** of exactly the approved permissions, ceiling-bounded, with a cache-invalidating revoke (§3) | — |
...
**Option (ii) — one-shot + TTL `LeasedDelegation` (recommended default).** On approval,
...
not co-drive), and the dead-man switch + immediate `revoke()` give one-shot + TTL semantics.
...
1. **The decision cache** — affects **every** permission, atomic or capability. The production
   ...
   cache ephemeral decisions at all** (see the dedicated subsection below): the policy reports an
   ephemeral decision only via `impliesEphemeral`, the SM consults it after a failed `implies`, and that result
...
2. **An escaped object reference** — capability-returning permissions only (`SocketPermission`→live
   ...
   each *use* re-checks. Because the ephemeral decision is reported by `impliesEphemeral` (never cached), that
...
`impliesEphemeral` cuts the decision channel and the method guard cuts the reference channel. The
...
**Ephemeral authority: `implies` vs `impliesEphemeral`.** The cacheability of a decision is a property of
...
- `implies(PD, perm)` — granted by **stable** authority. Ephemeral grants do **not** contribute.
- `impliesEphemeral(PD, perm)` — granted, but only by an **ephemeral** grant (a `LeasedDelegation` the
  EscalationGate flagged ephemeral). New method on `java.security.Policy`, default `return false`,
  overridden by `DynamicPolicyProvider`; callable from the java.base SM (no `RevocablePolicy`
  exposure). The grant declares ephemeral; the policy sorts it into this bucket.

The SM keeps its existing protected per-`(PD, perm)` check (the one `SecurityPolicyWriter`/polpAudit
hooks to **record** exercised permissions) **unchanged**, and simply **adds** an `impliesEphemeral`
consultation *after* a failed `implies`, outside that recording method. Per PD a domain grants if
`implies || impliesEphemeral`; the context is granted iff every PD grants; the context is **cacheable
iff no PD needed `impliesEphemeral`**. Sketch:

```
try {
    delegateContext.checkPermission(perm);   // implies-only; ephemeral grants invisible; recorded; cacheable
    checkedPerms.add(perm);                   // passed on stable authority → cache
    return;
} catch (AccessControlException e) {
    if (everyDomainGrantsVia(implies || impliesEphemeral, perm)) return;  // granted ephemeral, DON'T cache
...
1. **Never cached.** An ephemeral grant is reported only by `impliesEphemeral`, which the SM never writes to the
   cache. Zero overhead on the stable path — `impliesEphemeral` is consulted only when `implies` fails
   (an ephemeral grant, or a genuine denial that is already throwing). Passive lease *expiry* needs no
   trigger: `impliesEphemeral` re-reads liveness every call and the next check denies.
2. **Never recorded into generated policy** — the most important property. `SecurityPolicyWriter`
   ...
   ones at shutdown. That does **not** exclude an ephemeral grant on its own — and a shutdown-time filter
   cannot, because the ephemeral grant is gone by then. So the recorder makes the
   discrimination **at record time, while the grant is live**, with the same stable-first ordering
   the SM uses: it skips recording exactly when `!pd.implies(p) && pd.impliesEphemeral(p)` — stable
   denies but a live ephemeral grant permits — and still allows the operation (observe-only). Were an ephemeral grant
   ...
   *Operational contract:* this excludes only an ephemeral grant that is live at check time, i.e. the
...
3. **Safe to express as a `DelegatePermission`.** Because (2) keeps ephemeral grants out of the floor, an
   ephemeral capability can be a `DelegatePermission` without re-permanentising the very reference it
   exists to revoke; and `impliesEphemeral` is the same per-`(PD, Permission)` relation the
   `DelegateDomainCombiner` already evaluates the candidate at, so the method guard rides it per
   invocation. The ephemeral capability grant is then hack-free: a grant flagged ephemeral,
   permission = `DelegatePermission(candidate)`, reported only by `impliesEphemeral`, guarded per use.
...
when an ephemeral grant also covers it — the SM falls to `impliesEphemeral` (and skips caching) only when the
ephemeral grant is the *sole* grantor.

**Containment property (state it, don't trip over it).** Making `implies` blind to ephemeral grants means
ephemeral authority is honoured **only** through the cooperating SM's two-step
(`SecurityManager.checkPermission`). Code reaching `AccessController.checkPermission` directly
(bypassing the SM) sees only `implies` and *denies* the ephemeral grant. For an escalation this is correct
and fail-secure: every approved-operation guard routes through `sm.checkPermission` in production,
and an ephemeral grant deliberately cannot be exercised by code that goes around the authorization SM.

`clearCache()`/`refresh()` is **not** part of the ephemeral path; it keeps its existing role for the
...
**Survival across agent restore / replay (one-shot must be nonce-guarded, not merely timed).** The
...
nonce (`ApprovalId`), and `EscalationGate.request` consumes that nonce on first install and
...
public interface EscalationAuthority {
     ...
     *  approver Subject, and the approvalId ECHOED from the supplied nonce. An empty
     *  approved set is a denial. A non-empty approval is treated as DENY unless BOTH approver()
     *  is non-null (T2) AND approvalId() equals the supplied nonce (T3 nonce). The transport
    ...
    /** The approved subset + the authenticated principal + the nonce it is bound to. */
    final class Outcome {
...
public final class EscalationGate {
    public EscalationGate(RevocablePolicy policy, EscalationAuthority authority,
                      EscalationCatalogue catalogue, Duration leaseTtl,
     ...
     *  is denied before the human is prompted). Mints a nonce (ApprovalId), renders via the
     ...
     *  ceiling caps it (T2) — a one-shot + TTL LeasedDelegation of exactly the approved subset
     *  to request.agent(). The grant is flagged EPHEMERAL, so the policy reports it only via
     *  impliesEphemeral: never cached, never recorded into generated policy (§3). CONSUMES the ApprovalId
     *  (the nonce), and DELIVERS the handle to the DelegationSink. For a capability-returning
     *  permission the approved permission is a DelegatePermission guarded per-use, so the
     *  (uncached) impliesEphemeral re-check severs escaped references on expiry/revoke (T3). Emits an
...
- **Over-ask:** the human sees every requested token and approves a subset (per-Permission, §6-1);
  ...
  human.
- **Replay / standing-mode / restore (T3):** one-shot is enforced two ways. *Cache/expiry* — an
  ephemeral decision is reported only by `impliesEphemeral` and is **never cached**, so it is never
  ...
  and the uncached `impliesEphemeral` re-check severs an *escaped reference* on its next use.
  *Structurally* — each approval is bound to a nonce (`ApprovalId`) minted by `request` and
  ...
  an agent restore (§3).
- **Policy-recording leak (T3):** an ephemeral grant must never be written into generated least-privilege
  ...
  explicit **record-time** decision (a shutdown filter can't help: the ephemeral grant is gone by
  then). The recorder skips exactly when `!pd.implies(p) && pd.impliesEphemeral(p)` (stable denies, a live
  ephemeral grant permits) and still allows the op. Caveat: this catches only an ephemeral grant live
...
7. **One-shot is nonce-guarded and does not survive restore (added 2026-06-18).** Each approval is
   bound to a nonce (`ApprovalId`) minted by `request` and consumed on first install; a replayed
...
9. **Ephemeral decisions are never cached and never recorded — `implies` vs `impliesEphemeral` (revised
   ...
   `impliesEphemeral(PD, Permission)` (on `java.security.Policy`, default `false`, overridden by
   `DynamicPolicyProvider`) reports ephemeral authority. The SM leaves its protected per-PD recording
   check unchanged and **adds** an `impliesEphemeral` consultation *after* a failed `implies`, outside
   that method. One split, three properties: an ephemeral grant is never cached (no overhead on the stable
   ...
   skip when `!pd.implies(p) && pd.impliesEphemeral(p)` (stable denies, a live ephemeral grant permits), not a
   shutdown filter (the grant is gone by then); this keeps an escalation from being promoted to a
   permanent baseline grant (the worst failure mode). Only an ephemeral grant *live during the
   ...
   rides the uncached `impliesEphemeral` per use). The soundness corner resolves exactly — `implies`-first
   means a permanently-granted perm stays cached even if an ephemeral grant also covers it. *Supersedes the
   ...
   smart-proxy `DelegatePermission` discarded mid-life). Ephemeral authority is honoured only through
   the cooperating SM's two-step (`AccessController.checkPermission` bypassing the SM denies it —
   deliberate fail-secure containment). DirtyChai dependency: `impliesEphemeral` + the SM post-check land
   in java.base. (§1/§3/§5.)
...
- **C4a** (atomic, enforced path) Approved escalation is one-shot via the *enforced* check path:
  ...
  `CachingSecurityManager` so the never-cached `impliesEphemeral` path is exercised (see C11).
- **C4b** (capability) For a capability-returning permission approved as a `DelegatePermission`
  ...
  method call after `revoke()`/expiry — because the ephemeral decision is reported via `impliesEphemeral` and never
...
- **C11** (T3 ephemeral never cached, atomic) Under an active `CachingSecurityManager`: an approved
  ...
  reported by `impliesEphemeral`, which the SM does not cache), so the eventless expiry path is covered
...
- **C12** (T3 nonce / restore) Replaying a previously-approved `Outcome` (same `ApprovalId`)
...
- **C14** (`impliesEphemeral` coexists with a reusable `DelegatePermission` on one PD) A PD holds both a
  reusable smart-proxy `DelegatePermission` (stable, via `implies`) and an ephemeral `DelegatePermission`
  (via `impliesEphemeral`). Assert: the reusable one is served from cache on repeat checks (still viable);
  the ephemeral one is never cached and dies on expiry; neither taints the other. Confirms the marker
  granularity is the grant/`impliesEphemeral` relation, not the PD or the `DelegatePermission` class.
- **C15** (soundness corner) A perm covered by **both** a permanent grant (`implies`) and an ephemeral grant
  (`impliesEphemeral`) is served from cache and survives the ephemeral grant's expiry — `implies`-first means the
  stable grant wins and caching is correct; the ephemeral grant does not force non-caching of an
  independently-permanent decision.
- **C16** (no policy-recording leak, T3) Under `polpAudit` (`SecurityPolicyWriter`): an agent
  exercises an escalation **through the `EscalationGate`** (so the ephemeral grant is live during the
  ...
  (`!pd.implies(p) && pd.impliesEphemeral(p)`) excludes the live ephemeral grant while still capturing real floor
  gaps. **Negative control:** the same escalation perm exercised *without* the gate (no live ephemeral
