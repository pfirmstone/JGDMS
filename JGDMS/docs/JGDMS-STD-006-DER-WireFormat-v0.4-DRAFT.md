...
> **Editorial note (v0.4-DRAFT):** This is a working scaffold. Sections marked
...
>
> **Changes in v0.4 (from v0.3):**
> - New §7.7 — Jini Discovery/Registration Wire Types. ASN.1 modules for
>   `EntrySchemaRecord`, `EntryRecord`, `EntryTemplate`, `ServiceID`,
>   `ServiceItemRecord`, `ServiceTemplateRecord`, `ProxyDescriptor`
>   (with `JeriEndpointRecord` and `ServiceSpecRecord` CHOICE arms for
>   native JERI DER and proxy-factory-bridge patterns respectively),
>   and lease records. Hash algorithm modernised to
>   `SHA-256(DER(EntrySchemaRecord))` with legacy migration [OPEN].
> - §6 inventory updated with §7.7 types.
> - §10 open questions updated.
> - Appendix A updated: cross-reference to STD-005 DER appendix.
...
5. §7.3 — `DigestCodeSource` field confirmation (certificate encoding, DOS bounds as
   ...
   identity* only — it is NOT a stream annotation and carries no URL-locator role.
...
9. §7.7.1 — **Hash algorithm migration** (most consequential new item): settle
   Option A/B/C for coexistence of RULE-7 legacy hash with DER `SHA-256(DER(...))`.
   Recommendation: Option B (version tag in `EntryRecord`). Confirm before
   implementation — this affects all existing Registrars and all `@SerialEntry`
   classes.
10. §7.7.1 — Confirm whether array-valued `EntryWireField` types are permitted
    (e.g. `String[]`). If yes, encode as ORDER-SIGNIFICANT `SEQUENCE OF` per §3.8.
11. §7.7.4 — Confirm `ServiceSpecRecord` operation set is sufficient for all
    embedded device interface patterns in scope. Confirm whether `void` return type
    needs explicit representation in `TypeDescriptor`.
12. §7.7.5 — Confirm Jini spec attribute limit (64 entries per `ServiceItemRecord`
    as used in the schema `SIZE` constraint).
13. §7.7.5 — `requiredInterfaces`: confirm whether interface type identity uses the
    same `EntrySchemaRecord` hash scheme, or a distinct hash over the Java interface
    class name only.
14. §7.7.6 — Confirm `Lease.FOREVER` encoding: Java convention uses `-1` for
    "any duration" in `requestedDuration`; `Long.MAX_VALUE` for absolute expiry.
    The schema uses `LeaseForever` for both; confirm unambiguous representation.
15. Whole-document — validate every ASN.1 module against a real compiler
