...
> **Editorial note (v0.10-DRAFT):** This is a working scaffold. Sections marked
...
>
> **Changes in v0.10 (from v0.9):**
> - New §2.3 — Data Independence from Code: schema + DER bytes make all field
>   values permanently recoverable without the originating class. Data outlives
>   code; the schema is the universal key.
> - New §3.11 — Data Independence from Code: normative design principle derived
>   from §2.3. `MarshalledInstance` schema embedding is the mechanism that
>   guarantees this property.
> - New §7.8 — `MarshalledInstanceRecord`: DER wire type for `MarshalledInstance`.
>   Schema bytes are a required field, not optional. Schema digest included for
>   fast comparison. Codebase annotation optional (backup; `CodebaseAccessor` is
>   authoritative).
> - New §12 — Schema Registry Service: registry service interface, append-only
>   schema archival, `ServiceSchemaEntry` Jini Entry, and decision tree for schema
>   resolution.
