...
**Version:** 1.4 — `KEY_ALGORITHM_OID` corrected to `id-ecPublicKey`
(`1.2.840.10045.2.1`) — the existing x500 formats use signature algorithm OIDs
where key algorithm OIDs are required by `X509CertSelector.setSubjectPublicKeyAlgID()`;
this is a latent bug in the existing formats. New §8.4 documents the OID namespace
distinction. §10 item 4 promoted to a pre-implementation blocker. §15.5 cascade
...
class Constants {
    ...
    // id-ecPublicKey (RFC 5480) — the SubjectPublicKeyInfo algorithm OID for EC keys.
    // This is what X509CertSelector.setSubjectPublicKeyAlgID() matches against.
    // NOTE: The existing x500 formats incorrectly use SIGNATURE algorithm OIDs
    // (e.g. ecdsa-with-SHA512 = 1.2.840.10045.4.3.4) here — see §8.4 for the
    // bug analysis and the fix that should be applied to those formats.
...
class Constants {
    ...
    static final String KEY_ALGORITHM       = "EC";
    // id-ecPublicKey (RFC 5480) — same rationale as SHA256 variant.
    static final String KEY_ALGORITHM_OID   = "1.2.840.10045.2.1";
    private Constants() {}
}
```
...
### 8.4 `KEY_ALGORITHM_OID` — Signature OID vs Key OID Bug (pre-implementation blocker)

**This is the most important thing to verify before writing any cert-lookup code.**

An X.509 certificate contains two OID fields that are easy to confuse:
...
Certificate
  └── TBSCertificate
        ...
        │     └── algorithm
        │           └── algorithm OID  ← KEY OID: what setSubjectPublicKeyAlgID() matches
        └── signatureAlgorithm         ← SIGNATURE OID: entirely different arc
```

`X509CertSelector.setSubjectPublicKeyAlgID(String oid)` matches against the
**SubjectPublicKeyInfo** algorithm OID — the key OID, not the signature OID.

**The existing x500 formats use the wrong OID arc:**

| Format | Current `KEY_ALGORITHM_OID` | Arc | What it actually identifies |
|---|---|---|---|
| `SHA256withDSA` | `2.16.840.1.101.3.4.3.2` | `2.16.840.1.101.3.4.3.*` | `id-dsa-with-sha256` — a **signature** OID (RFC 5758) |
| `SHA512withECDSA` | `1.2.840.10045.4.3.4` | `1.2.840.10045.4.*` | `ecdsa-with-SHA512` — a **signature** OID (RFC 5758) |

**The correct values (key OIDs, RFC 3279 / RFC 5480):**

| Key type | Correct `KEY_ALGORITHM_OID` | Standard |
|---|---|---|
| DSA | `1.2.840.10040.4.1` | `id-dsa` (RFC 3279) |
| EC  | `1.2.840.10045.2.1` | `id-ecPublicKey` (RFC 5480) |

**Impact on existing formats:** If `X500Provider.getCertificate0()` passes
`keyAlgorithmOID` directly to `setSubjectPublicKeyAlgID()`, the cert selector
never matches any certificate — no standard X.509 certificate carries a signature
algorithm OID in its SubjectPublicKeyInfo. The cert lookup silently returns null,
and multicast announcement verification fails. Whether this is a live or latent bug
depends on whether `X500Provider` falls back to subject-name-only lookup when the
selector returns nothing. **Read `X500Provider.getCertificate0()` first.**

**Impact on new SPIFFE formats:** `SpiffeX500Provider` overrides `getCertificate()`
entirely with PKIX CA-chain validation. The OID never reaches
`setSubjectPublicKeyAlgID()` at runtime. Nevertheless, using the correct OID
(`1.2.840.10045.2.1`) is mandatory:
- Documents the correct intent
- Avoids a latent trap if the base class path is ever hit
- Does not replicate the existing bug

**Action before Phase 2 implementation:**
1. Read `X500Provider.getCertificate0()` — confirm whether `keyAlgorithmOID` is
   passed to `setSubjectPublicKeyAlgID()` or used differently.
2. If it IS passed to `setSubjectPublicKeyAlgID()`:
   - Fix `x500/sha256withdsa/Constants.java`: `KEY_ALGORITHM_OID = "1.2.840.10040.4.1"`
   - Fix `x500/sha512withecdsa/Constants.java`: `KEY_ALGORITHM_OID = "1.2.840.10045.2.1"`
   - These are one-line fixes but should be in a separate commit with a clear comment
3. If it is NOT passed to `setSubjectPublicKeyAlgID()` (used differently), document
   how it IS used and confirm the new SPIFFE format value is still correct.

The new SPIFFE formats use `"1.2.840.10045.2.1"` regardless — this value is
correct by RFC 5480 and is not affected by what the existing formats do.

### 8.5 `MAX_SIGNATURE_LEN` arithmetic in SigningBufferFactory
...
### 8.6 Revocation must be disabled for PKIX validation
...
### 8.7 `writeClassAnnotationCerts` / `readAnnotationCertsGrantPerm` MUST NOT be called
...
### 8.8 `SvidRotationListener` is for trust bundle refresh, not SVID rotation
...
### 8.10 `checkAuthenticationPermission` must be called
...
4. **`KEY_ALGORITHM_OID` bug in existing formats — PRE-IMPLEMENTATION BLOCKER**
   Read `X500Provider.getCertificate0()` before writing any cert-lookup code.
   Confirm whether `keyAlgorithmOID` is passed to `setSubjectPublicKeyAlgID()`.
   If so, fix the existing format Constants (see §8.4 for correct values and
   fix procedure). This must be done before Phase 2, as `SpiffeX500Provider`
   extends `X500Provider` and must not inherit broken cert-selector behaviour.
...
| `x500/sha256withdsa/Constants.java` | Fix OID: `KEY_ALGORITHM_OID = "1.2.840.10040.4.1"` (`id-dsa`) — **if** `getCertificate0()` passes it to `setSubjectPublicKeyAlgID()` (see §8.4) |
| `x500/sha512withecdsa/Constants.java` | Fix OID: `KEY_ALGORITHM_OID = "1.2.840.10045.2.1"` (`id-ecPublicKey`) — same condition |
