...
> **Editorial note (v0.7-DRAFT):** This is a working scaffold. Sections marked
...
>
> **Changes in v0.7 (from v0.6):**
> - §11 — New closing section §11.8 *Symmetric Graceful Degradation* added before
>   the summary table (now §11.9). Explains why the namespace privacy rules produce
>   safe forward and backward compatibility in both directions: new classes receive
>   defaults; absent classes leave their wire data unread. The apparent strictness
>   of the namespace rules is the mechanism that enables this property.
...
Absent fields on deserialisation — because the class did not exist when the data was
written, or because the field was added in a later version — return their declared
default values via `GetArg.get(name, defaultValue)`. The constructor is then
responsible for determining whether those defaults satisfy the class invariants:
replacing them with derivable valid values if possible, or throwing if not. This is
the only migration mechanism. No silent data loss or silent incorrect reconstruction
is permitted.
