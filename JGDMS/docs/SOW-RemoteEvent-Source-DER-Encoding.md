...
**STATUS: READY TO IMPLEMENT — design revised 2026-07-19 in response to the board review,
independently re-reviewed 2026-07-19 (second pass, verdict SOUND).** The original chosen fix
(wrap `source` in a hard-coded-`ATOMIC_DER` `MarshalledInstance`) is **withdrawn**: board
Findings 2 and 3 (see "Board review findings" below) showed it fails its own acceptance test
and replicates a live deserialization-downgrade hazard. The revised design ("Chosen fix"
below) routes `source` through the STD-006 `Any` form instead — no `MarshalledInstance` on the
wire for `source` at all. Findings 1–4 are each explicitly resolved in "Resolution of board
findings"; a second, independent reviewer then re-derived every load-bearing claim against
current source and confirmed the redesign is genuinely different from (and safer than) both
the withdrawn wrap and the earlier-rejected Alternative 3 — see "Second board review findings"
at the end of this document. That second pass found one real MEDIUM finding (already folded
into "Concrete change" item 4 below — a reach-widening/forcing-function gap) and two LOW
precision fixes (already applied inline: the exact `SchemaGenerator` patch location in item 1,
and a correction to which sites the subclass-narrowing pattern actually applies to). The
revised design has been verified against trunk source by two independent reads (every file/line
cited below re-read 2026-07-19) but **not yet built or executed** — empirical verification is
the implementing agent's first task.
