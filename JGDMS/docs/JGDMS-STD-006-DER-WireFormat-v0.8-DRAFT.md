...
> **Editorial note (v0.8-DRAFT):** This is a working scaffold. Sections marked
...
>
> **Changes in v0.8 (from v0.7):**
> - §3.9 — Both field-level directions of the symmetric degradation property are
>   now explicitly stated as normative rules: a field absent from the wire returns
>   its declared default; a field present on the wire but not requested by the
>   constructor is silently discarded. The constructor is the sole arbiter of what
>   it reads from `GetArg`.
> - §11.8 — Closing paragraph added noting that the symmetric degradation property
>   is uniform across all granularities: field-level changes within a class and
>   class-level changes within a hierarchy follow exactly the same rules.
...
**Absent field — requested but not present on the wire.** A field that appears in
`serialForm()` but is absent from the received SEQUENCE — because the field was added
in a later version, or because the class did not exist when the data was written —
returns its declared default value via `GetArg.get(name, defaultValue)`. The
constructor is then responsible for determining whether that default satisfies the
class invariants: replacing it with a derivable valid value if possible, or throwing
if not.

**Present field — on the wire but not requested by the constructor.** A field that
appears in the received SEQUENCE but is never read by the constructor via
`GetArg.get()` is silently discarded. The constructor makes no obligation to consume
every field in its SEQUENCE. Fields present in older or richer wire data that the
current constructor does not request have no effect on reconstruction.

These two rules are symmetric and apply uniformly at the field level within a single
class's namespace. Adding a field to `serialForm()` is safe: old wire data returns
the default, new wire data supplies the value. Removing a field from `serialForm()`
is safe: old wire data that still carries the field is never requested and is
silently discarded.
...
constructor then determines whether those defaults satisfy its invariants — replacing
them with derivable valid values if possible, or throwing if not. No external
coordination is needed. The class handles its own absence through the same invariant
path it uses for any other absent optional field.
...
hierarchy. Its SEQUENCE is present in the wire and available in `GetArg`, but
nothing reads it — there is no `(GetArg)` constructor to consume those fields.
The data sits unread. The remainder of the hierarchy processes its own SEQUENCEs
normally. No error occurs, no data belonging to other classes is disturbed, and
the unread fields are silently discarded.
...
consume another class's fields. A class whose wire data is present but unread cannot
corrupt another class's defaults or state. The namespace walls are what make both
...
belonging to an existing class, or an absent class's unread data might be
...
that is absent from old wire data returns its declared default — the constructor
handles this via invariant checking. A field removed from `serialForm()` that is
still present in old wire data is simply not requested via `GetArg.get()` and is
silently discarded. Adding or removing a field within a class is therefore exactly
analogous to a class appearing in or disappearing from the hierarchy: in both cases,
the party with less information receives defaults; the party with more information
ignores what is not asked for. The model makes no distinction between these
granularities. The rule is uniform: *absent fields return defaults; unrequested
...
| Change | Wire impact | Migration required |
|---|---|---|
| Add non-`@AtomicSerial` subclass | None | No |
| Remove non-`@AtomicSerial` subclass | None | No |
| Add non-`@AtomicSerial` superclass | None to wire format; `@AtomicSerial` child may add fields | OPTIONAL + DEFAULT for new fields |
| Add `@AtomicSerial` to superclass | New SEQUENCE on wire; existing child SEQUENCE unchanged | None for existing data; defaults + invariant check for new SEQUENCE |
| Remove `@AtomicSerial` from class | SEQUENCE disappears; child takes responsibility | OPTIONAL + DEFAULT if child adds formerly-parent fields |
| Insert new `@AtomicSerial` class | New SEQUENCE; existing SEQUENCEs unchanged | OPTIONAL + DEFAULT in new SEQUENCE |
| Remove `@AtomicSerial` class | SEQUENCE disappears; neighbouring classes unaffected | None required |

The common thread: each namespace evolves independently. No change to one class's
SEQUENCE requires a corresponding change to another class's SEQUENCE. The only
migration mechanism is OPTIONAL fields with DEFAULT values, processed through
constructor invariant checking.
