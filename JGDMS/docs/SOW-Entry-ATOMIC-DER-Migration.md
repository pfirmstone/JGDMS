...
- **Supersession note (2026-07-21):** for **Outrigger only**, §3.5's born-immutable
  format-*choice* regime is superseded (strengthened) by
  `SOW-Outrigger-DER-Only-JOSS-Rejection.md` — JGDMS 4.0.0 Outrigger is `ATOMIC_DER`-only;
  no born-JOSS Outrigger exists. Reggie (A3) is unaffected. A5's output now targets that
  SOW's Unit-2 container format.
...
5. **Migration strategy — RATIFIED (Peter, 2026-07-20): born-immutable per-deployment format;
   no in-place migration of a running store.** *(2026-07-21: for Outrigger the choice is
   superseded — born format is unconditionally `ATOMIC_DER` per
   `SOW-Outrigger-DER-Only-JOSS-Rejection.md`; the choice regime survives only for
   Reggie/A3.)* A space/registrar's marshalling format is fixed at
...
| Task | Deliverable |
|------|-------------|
| **A1** · Outrigger DER marshal path + format selection | **[LANDED (Outrigger); hardened to DER-only by `SOW-Outrigger-DER-Only-JOSS-Rejection.md`]** Constraints-aware marshalling in `EntryRep` (both the reflection path `:338` and the `@SerialEntry` path `:406`) that marshals in **the space's own configured format** — advertised via the exported proxy's `MarshallingFormat` constraint and inherited uniformly by every client of that space (§3.5) — reusing the `MarshallingFormat`/`chooseMarshalFactory` plumbing; **no** per-relationship format derivation and **no** dual-format path. Includes the proxy-side template path (`SpaceProxy2.repFor`) so templates are produced in that **same single** born format as the entries they match. **Guard (i) in scope (§3.5):** the marshal path MUST refuse any attempt to change format on an existing populated store — immutability enforced, not assumed. Plus the loud-failure rule for non-DER-encodable field values (§3.6). **Additional marshal sites** (missing from earlier drafts): Outrigger `SpaceProxy2.java:549,699` (handbacks), `AvailabilityRegistrationWatcher.java:406`, `EventRegistrationWatcher.java:207`; Reggie `Item.java:243-246`, `EventReg.serialize`/`writeObject`. **Handback resolution (2026-07-20, implementation + review):** the four Outrigger handback sites **stay opaque JOSS by nature and are correctly left unchanged** — each re-wraps an already-serialized `java.rmi.MarshalledObject` (the legacy `JavaSpace`/pre-`JavaSpace05` event-delivery API), which carries no format to thread and cannot be re-marshalled without deserializing (forbidden); handbacks are event-delivery payload, never byte-matched, so no format-consistency requirement applies. They are out-of-scope by construction, not unfinished work. Most materially, `RegistrarImpl.marshalAttributes` (`:4447-4456`) and `marshalLocators` (`:4492-4501`) are both JOSS-only today and both called from inside `takeSnapshot` (`:6368-6369`) — the persistence path §2.3 already cites — so they belong in A4's single-format recovery tests, not silently assumed covered by A3's config flip. **Also in scope:** the DER-encodability fix for the two shipped failing fields the full survey found (§3.6) — `Status.severity` (mechanical `@AtomicSerial` retrofit on `StatusType`) and `UIDescriptor.factory` (`MarshalledObject`, a flagged compatibility-gated decision, not a mechanical fix). |
| **A2** · Born-immutable format design + guards | **[LANDED (Outrigger); hardened to DER-only by `SOW-Outrigger-DER-Only-JOSS-Rejection.md` — the format choice and guard (i) are superseded for Outrigger; guard (ii) hardens to unconditional JOSS refusal]** The format-at-instantiation config surface per service (format fixed at instantiation, immutable for the instance's life, §3.5); the exported-proxy `MarshallingFormat` constraint advertisement that makes that format the space-wide epoch every client inherits; the **two immutability guards** — (i) refuse an in-place format change on a populated store, (ii) refuse a snapshot on recovery whose format contradicts the instance's configuration; the **DER space registers only with a DER Reggie** deployment rule; and operator-facing guidance that "migration" = **redeployment** (stand up new DER instances, retire old JOSS ones — never an in-place flip). Plus `useDerForEntries` documentation (currently undocumented outside code). The withdrawn coexistence strategies (lease-drain, transitional dual-format, cold-start) and their unsoundness are recorded in §3 item 5 and are **not** built. Decision doc + config/recovery changes, board-reviewed. |
| **A3** · Reggie born-immutable format epoch + born-DER default | A new (JGDMS 4.0.0+) Reggie is configured `ATOMIC_DER` **at instantiation** — the new-deployment default is DER — **not** an in-place flip of a running registrar's `useDerForEntries`; the JOSS path remains only for running as, or interoperating with, prior-version djinns. **Structurally parallel to A1, NOT a light default-flip (investigation 2026-07-20, correcting an earlier draft of this row).** Reggie's *current* per-field format decision is `Util.requiresDerFormat(this)` reading the **client's own proxy** `getConstraints()` (`RegistrarProxy.java:150…`, `Registration.java:190…`) — a client-mutable, **silently-droppable** value (any client `ProxyPreparer.setConstraints()` wholesale-replaces it, flipping the format back), with **no** registrar-side non-droppable advertisement. That is *weaker* than per-relationship and is squarely inconsistent with §3.5's born-immutable epoch; the earlier "format is decided entirely client-side… set at instantiation, not flipped mid-life" framing was self-contradictory (a droppable client-side field is not "set at instantiation") and is **withdrawn**. A3 must therefore do the same work A1 did for Outrigger: (i) `useDerForEntries` becomes the **whole-instance** born format (self-attributes *and* every registrant's attributes *and* every lookup template), read once at startup; (ii) an **immutable, non-`getConstraints()`-derived `entryFormat` field** on `RegistrarProxy`/`Registration` (and their `Constrainable*` subclasses + `Item`), carried over unconditionally through every `setConstraints()` so a client cannot drop it — the exact A1 non-droppability fix; (iii) every call site switches from `Util.requiresDerFormat(this)` to reading that field (Util retired as the *decision* maker, kept at most as a diagnostic); (iv) conditional `AtomicDerILFactory` + `MarshallingFormat.ATOMIC_DER` exporter constraint as the wire-layer belt-and-braces (Reggie's exporter is unconditionally `AtomicILFactory` today); (v) a persistence recovery guard mirroring A1's snaplogstore guard. The existing `useDer` boolean-threading through `EntryRep.marshal`/`Item`/`Template` is reused (the one genuine saving vs A1); the boxed-scalar codec fix (`c70cc8fa2`) already covers Reggie's numeric attribute fields. **Recovery guard — airtight given DER-unreleased:** A1's "absent format marker ⇒ implicit JOSS" recovery rule is safe for Reggie too, because **DER is unreleased** (Peter, 2026-07-20) — no deployed store contains DER-encoded entries, so an unmarked pre-A3 snapshot is provably pure JOSS (the `reggie-der.config` DER sample is unreleased demo config, not a released-compat surface). **Behavior-change note:** retiring the per-relationship mechanism changes behavior for any deployment using the unreleased `reggie-der.config` pattern — release-note it, but it is not a released-compat break (DER unreleased). **Client-reach:** a DER Reggie serves only DER-capable clients; a JOSS-only client uses the prior-version djinn — the intended deployment boundary (§3.5), and a format-mismatched client fails **loud** (`UnsupportedConstraintException` via `chooseMarshalFactory`), never a silent no-match. Ties to §9.2 (resolved: a registrar-wide format epoch fixed at instantiation). |
...
6. **EntryRep v2 — the whole-entry DER record.** One record per entry (single schema chain,
   ...
   cheap interim. *(2026-07-21: deferral re-affirmed with corrected rationale in
   `SOW-Outrigger-DER-Only-JOSS-Rejection.md` §6.1 — the future migration is a
   schema-chain-carrying **verbatim** transcription, not a decode/re-encode; a
   layout-derivability invariant on v2 is recorded there.)*
