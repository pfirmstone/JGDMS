...
| `SpiffeLoginModule` | JGDMS JERI | Reads PEM files from the SPIRE-written directory (`net.jini.jeri.ssl.spiffe.dir` + `serviceRole` JAAS option). Populates `Subject` with `X500Principal` (cert Subject DN) and `SpiffePrincipal` (URI SAN). Used for services that authenticate via traditional JERI TLS. | ✅ Complete |
