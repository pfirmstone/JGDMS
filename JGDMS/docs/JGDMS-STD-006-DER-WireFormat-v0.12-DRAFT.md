...
> **Editorial note (v0.12-DRAFT):** This is a working scaffold. Sections marked
...
**As-built byte framing (`RemoteContextCodec` — normative for the current transport).**
Written through the proxy's injected `ObjectOutput`/`ObjectInput`, so the payload is DER
under a DER-exported proxy and JOSS otherwise, decoded in its own isolated codec stream
(no shared decode state, handle table, or DoS budget with the application arguments):
