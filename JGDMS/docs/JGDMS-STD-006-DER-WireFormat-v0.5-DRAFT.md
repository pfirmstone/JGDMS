...
> **Editorial note (v0.5-DRAFT):** This is a working scaffold. Sections marked
...
>
> **Changes in v0.5 (from v0.4):**
> - New §7.7.7 — Multicast Discovery Wire Types. ASN.1 modules for
>   `MulticastAnnouncementRecord` and `MulticastRequestRecord`. Defines two
>   format variants: `net.jini.discovery.x500.SHA256withECDSA` (trust-store
>   model, leaf cert absent from packet) and `net.jini.discovery.spiffe.*`
>   (PKIX CA-chain model, leaf cert in packet). Documents the SPIFFE Subject DN
>   non-empty requirement and the ECDSA P-256 signature length.
> - New §7.7.8 — Unicast Discovery Response Wire Type. ASN.1 module for
>   `UnicastResponseRecord`. Explicitly records deprecation of the
>   `writeClassAnnotationCerts`/`readAnnotationCertsGrantPerm` in-band cert-grant
>   mechanism and its replacement by `ProxyDescriptor` + `VerdictRegistry` path.
> - §6 inventory updated with §7.7.7 and §7.7.8 types.
> - §10 open questions updated: MTU constraint and `getTrustBundle()` API items
>   added; ECDSA format name item added.
> - Appendix A: agent context document cross-reference added.
