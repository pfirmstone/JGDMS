...
>   re-analysis) immediately after downloading a JAR and before computing the
>   content hash. The `contentHash` field in `AnalysisRequest` is now defined as
>   SHA-256 of the **normalised** JAR bytes, not the raw downloaded bytes.
> - **`contentHash` = `javaCodeDigest`.** The normalised-JAR SHA-256 hash is now
>   the same `javaCodeDigest` used by JGDMS-STD-007 (CodebaseCompatibilityInterchange)
>   as the primary key for language-native artifact lookup. One hash, consistent
>   identity across SCAP and STD-007.
> - **`AnalysisRequest` data object** updated: `packedJarBytes` now explicitly
>   carries Pack200-compressed **normalised** JAR bytes; `contentHash` definition
>   updated to reflect normalised form.
> - **Host 4 workflow** updated: normalisation step inserted between download and
>   hash computation.
> - **Full Verdict Flow** updated to show normalisation step.
> - **Key Design Decisions** table: normalisation row added.
> - **New section:** Relationship to JGDMS-STD-007.
...
round-trip at `EFFORT=1` followed by ZIP canonicalisation (entry sorting, mtime
pinning, extra-field stripping, deflate unification). The resulting JAR `C` is a
Pack200 fixed point: semantically identical to `B` but byte-reproducible across
independent builds. The SHA-256 of `C` is stable and suitable for content-addressed
identity. Without normalisation, ZIP mtime variance between builds would produce
different hashes for the same semantic JAR.
...
Decompression happens exactly once inside `AnalysisRequest.check(GetArg)`. The BAE
verifies `SHA-256(jarBytes) == contentHash` during validation. Since both the hash
and the packed bytes are derived from the same normalised form, this check is
guaranteed to pass for well-formed requests.

The bridge constructor receives the raw bytes directly — no double unpacking.
...
Host 4 (CD): check Host 3 cache by hash — skip if already known
     │
     ▼
Host 4: download JAR bytes B
Host 4: Normalize.normalize(B) → C  (canonical, byte-reproducible form)
Host 4: compute SHA-256 hash H of C  (contentHash = javaCodeDigest)
     │
     ▼
AnalysisRequest (Pack200-compressed(C), H, bfsDepth, codebaseUrls)
...
BAE #1 analyzeJar()     BAE #2 analyzeJar()  ...  BAE #N analyzeJar()
  ...
  (BAE-1-signed)           (BAE-2-signed)            (BAE-N-signed)
     ...
     └──────── Host 4 → VR.submitReport(engineId, report) ──────────┘
                        ...
                        applyQuorumPolicy()
                              │
                        RegistryVerdict (Host-3-signed, keyed by H)
                              │
                  ┌───────────┴──────────────┐
                  │                          │
...
| `Normalize.normalize()` before hash computation | Removes ZIP mtime variance, entry-ordering variance, and classfile attribute non-determinism from the hash input. The same semantic JAR produces the same `contentHash` regardless of when or where it was built. Required for content-addressed identity and for alignment with JGDMS-STD-007's `javaCodeDigest`. |

---

## Relationship to JGDMS-STD-007

JGDMS-STD-007 (CodebaseCompatibilityInterchange) defines a service that compiles
SCAP-verified JARs to language-native artifacts (shared libraries, WebAssembly
modules) for non-JVM clients.

**`contentHash` = `javaCodeDigest`.** The `contentHash` computed by SCAP (SHA-256
of the normalised JAR, as defined in this standard v1.2) is the same value that
STD-007 uses as `javaCodeDigest` — the primary key for all artifact lookups in the
CodebaseCompatibilityInterchange service. A single normalised JAR therefore has one
stable identity across both pipelines:

- SCAP uses it to key `RegistryVerdict` — "is this JAR safe to unmarshal?"
- STD-007 uses it to key compiled artifacts — "what language-native proxy is
  available for this JAR?"

**Pipeline ordering.** For a JAR that needs both SCAP analysis and STD-007
compilation, the normalisation step (see Host 4 workflow) produces the canonical
form `C` from which both `contentHash`/`javaCodeDigest` and the SCAP
`AnalysisRequest` are derived. The STD-007 compilation pipeline receives the same
normalised JAR `C` and produces artifacts identified by `SHA-256(C)`. No separate
normalisation step is needed for STD-007.
