...
/**
 * Pins the backend-interface convention for BytecodeAnalysisEngine: a JERI stub
 * for {@link BytecodeAnalysisEngineBackend} transitively implements exactly the
 * contract {@code AbstractSmartProxy.checkServer} requires, so the exported stub
 * passes deserialization and admin operations are invocable over the wire.  The
 * AtomicSerial marshal/checkServer mechanics themselves are proven generically
 * by the hello-world example's round-trip test.
 */
public class BytecodeAnalysisEngineBackendContractTest {

    @Test
    public void testBackendAggregatesCheckServerContract() {
        Class<?> b = BytecodeAnalysisEngineBackend.class;
        for (Class<?> required : new Class<?>[]{
                Remote.class, BytecodeAnalysisEngine.class,
            ...
            assertTrue(required.getName()
                    + " must be aggregated by BytecodeAnalysisEngineBackend",
