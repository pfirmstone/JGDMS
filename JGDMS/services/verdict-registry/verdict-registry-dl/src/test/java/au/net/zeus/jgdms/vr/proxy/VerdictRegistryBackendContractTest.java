...
/**
 * Pins the backend-interface convention for VerdictRegistry: a JERI stub for
 * {@link VerdictRegistryBackend} transitively implements exactly the contract
 ...
 */
public class VerdictRegistryBackendContractTest {

    @Test
    public void testBackendAggregatesCheckServerContract() {
        Class<?> b = VerdictRegistryBackend.class;
        for (Class<?> required : new Class<?>[]{
                Remote.class, VerdictRegistry.class,
            ...
            assertTrue(required.getName()
                    + " must be aggregated by VerdictRegistryBackend",
