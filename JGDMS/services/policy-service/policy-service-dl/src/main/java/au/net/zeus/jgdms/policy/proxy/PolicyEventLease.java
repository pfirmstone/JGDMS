 ...
 * {@link RemotePolicyService} event-listener registration.
 *
 ...
 * {@link RemotePolicyService#registerForPolicyUpdates}.  Clients use the
 * standard {@link Lease} interface; the implementation forwards renewals and
 * cancellations back to the server via the
 * {@link RemotePolicyService#renewPolicyLease} and
 * {@link RemotePolicyService#cancelPolicyLease} remote methods.
 *
 * @see RemotePolicyService#registerForPolicyUpdates
 * @since 3.1.1
 */
@AtomicSerial
public final class PolicyEventLease extends AbstractLease {
    ...
    public static SerialForm[] serialForm() {
        return new SerialForm[]{
            new SerialForm(SERVER,   RemotePolicyService.class),
            new SerialForm(LEASE_ID, Uuid.class)
        };
    }

    public static void serialize(PutArg arg, PolicyEventLease l) throws IOException {
        arg.put(SERVER,   l.server);
        arg.put(LEASE_ID, l.leaseId);
        arg.writeArgs();
    }

    /** Remote reference to the RemotePolicyService server. @serial */
    private final RemotePolicyService server;
    ...
    private static GetArg check(GetArg arg)
        ...
        RemotePolicyService srv = arg.get(SERVER, null, RemotePolicyService.class);
     ...
     */
    public PolicyEventLease(GetArg arg)
            throws IOException, ClassNotFoundException {
        super(check(arg));
        this.server  = arg.get(SERVER,   null, RemotePolicyService.class);
        this.leaseId = arg.get(LEASE_ID, null, Uuid.class);
    }

    /**
     * Constructs a {@code PolicyEventLease}.
     *
     * @param server     remote reference to the RemotePolicyService; must be
     *                   non-null
     ...
     *                              {@code null}
     */
    public PolicyEventLease(RemotePolicyService server,
                             Uuid leaseId,
                             long expiration) {
        super(expiration);
    ...
    protected long doRenew(long duration)
        ...
        return server.renewPolicyLease(leaseId, duration);
    }

    @Override
    public void cancel()
            throws UnknownLeaseException, RemoteException {
        server.cancelPolicyLease(leaseId);
    }

    /**
     * Two {@code PolicyEventLease} instances can be batched if they refer to
     * the same server endpoint.
     */
    @Override
    public boolean canBatch(Lease lease) {
        return (lease instanceof PolicyEventLease)
                && server.equals(((PolicyEventLease) lease).server);
    }

    @SuppressWarnings("unchecked")
    @Override
    public LeaseMap createLeaseMap(long duration) {
        return new PolicyLeaseMap(this, duration);
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof PolicyEventLease)) return false;
        PolicyEventLease other = (PolicyEventLease) obj;
        return leaseId.equals(other.leaseId);
    }

    // -------------------------------------------------------------------------
    // Inner class: batch lease map
    // -------------------------------------------------------------------------

    /**
     * {@link LeaseMap} implementation for batching {@link PolicyEventLease}
     * renewals and cancellations.
     */
    @SuppressWarnings({"unchecked", "rawtypes"})
    private static final class PolicyLeaseMap extends AbstractLeaseMap {

        PolicyLeaseMap(PolicyEventLease lease, long duration) {
            super(lease, duration);
        }

        @Override
        public boolean canContainKey(Object key) {
            if (!(key instanceof PolicyEventLease)) return false;
            Iterator<?> it = map.keySet().iterator();
            if (!it.hasNext()) return true;
            PolicyEventLease first = (PolicyEventLease) it.next();
            return first.server.equals(((PolicyEventLease) key).server);
        }
        ...
        public void renewAll() throws LeaseMapException, RemoteException {
            ...
            for (Object obj : map.entrySet()) {
                ...
                PolicyEventLease lease    = (PolicyEventLease) entry.getKey();
        ...
        public void cancelAll() throws LeaseMapException, RemoteException {
            ...
            for (Object keyObj : new ArrayList<Object>(map.keySet())) {
                PolicyEventLease lease = (PolicyEventLease) keyObj;
                try {
                    lease.cancel();
                    map.remove(lease);
                } catch (Exception e) {
