...
public abstract class Discovery {
     ...
     * named
     * <code>"META-INF/services/org.apache.river.discovery.DiscoveryFormatProvider"</code>
     ...
     * determined by interpreting any resources of the indicated class loader
     * named
     * <code>"META-INF/services/org.apache.river.discovery.Discovery"</code>
     ...
     * @param loader class loader from which to load providers, or
     * <code>null</code> to indicate the current context class loader
     * @return an instance implementing the unicast https protocol
     */
    public static Discovery getUnicastHttps(final ClassLoader loader){
	return AccessController.doPrivileged(
        new PrivilegedAction<Discovery>(){
            @Override
            public Discovery run() {
		ClassLoader ploader;
		if (loader == null){
		    ploader = Thread.currentThread().getContextClassLoader();
		} else {
		    ploader = loader;
		}
                ServiceLoader<Discovery> serviceLoader 
                   = ServiceLoader.load(Discovery.class, ploader);
                Iterator<Discovery> iter = serviceLoader.iterator();
                Discovery spi;
                try {
                    while ( iter.hasNext() ) {
                        try {
                            spi = iter.next();
                            if (spi != null && spi.toString().equals("UnicastHTTPS")) {
                                logger.log(Level.CONFIG, "loaded: {0}", spi.getClass().getName());
                                return spi;
                            }
                        } catch (Throwable e) {
                            logger.log( 
                                    Level.CONFIG, 
                                    "error loading Discovery: {0}",
                                    new Object[]{e}
                            );
                        }
                    }
                } catch (Throwable t) { }
                logger.log(Level.CONFIG, "uable to find Discovery Provider");
                return null;
            }
        });
    }
